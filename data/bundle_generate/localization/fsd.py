...
async def load_localization(index: ResourceTree, localization_root: Path):
    LOGGER.info("Loading fsd localization files...")

    en = await index.download_resource(LOC_EN_RES)
    if not en:
        LOGGER.warning(f"Localization resource '{LOC_EN_RES}' not found.")
        return

    zh = await index.download_resource(LOC_ZH_RES)
    if not zh:
    ...
    bundle_loc_pb = localization_root / "localizations.pb"
    ...
    LOGGER.info("Created fsd localization protobuf file.")
