...
async def main():
    ...
    group.add_argument("--workspace", "-w", type=str, help="Specify workspace name to process")
    group.add_argument("--all", "-a", action="store_true", help="Process all available workspaces")
    group.add_argument(
        "--list", "-l", action="store_true", help="List available workspaces and exit"
    )
    group.add_argument(
        "--clean-bundles", action="store_true", help="Clean up existing bundles before processing"
    )
    group.add_argument(
        "--clean-cache", action="store_true", help="Clean up existing cache before processing"
    )
    group.add_argument(
    ...
    if args.all:
        ...
        for workspace in workspaces:
            processor = BundleGenerator(workspace)
            processor.clean_all_cache()
